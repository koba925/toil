...
[^2ed]:「350行くらいのPythonで作るプログラミング言語実装超入門（第2版）」くらいのつもりで考えてたんですがもう全然別物になったのでタイトルも変えました。
...
第1章が終わるともうこんなプログラム（？）が動かせるようになります。
...
* 第1部 最小のインタプリタ
  ...
  * 第2章：最小の字句解析・構文解析器 (300行)
    テキストのソースコードを読んでASTに変換し、第2章の評価器で読めるようにします。
    これでもう小さなインタプリタになっています。
  * 第3章：最小の中間コードインタプリタ [^bytecode-interpreter]  (500行)
    第2章の字句解析・構文解析器とASTを中間コードに翻訳するコンパイラ、その中間コードを実行するVMの組み合わせによるインタプリタを作ります。
...
この書籍では、第1部を読むだけでインタプリタ処理系の全体像がつかめるようになっています。
ざーっと第1部の最後まで眺めたら、第2章を読んで書いてみる、3章を読んで書いてみる、のように小刻みに進めてみるとよいと思います。
なんなら第2章だけ読んで書いてみる、でも。

第1部を理解できればもう自分で言語を拡張したり、1から自分の言語を書いたりすることもできます。
第2部以降を読んで書いてみるというのはもちろんですが、先に自分で書いてみるのも面白いかと。
...
* メモリ管理
  ...
  そのあたりはPythonにまかせています。

[^gc]: ガベージコレクション（Garbage Collection: GC）と呼ばれます。できることならやりたい話題なんですが処理系を根底から書き直すことになるので見送っています。もともと性能は度外視しているとはいえ、Pythonではさすがに処理速度的にも厳しいかなあ・・・
...
GitHubにはテスト用のコードも入れていますが、この本ではテストコードについては解説しません。
...
ちゃんとした処理系では、エラーの出力は使い勝手に大きくかかわる重要な機能で、わかりやすいメッセージを出したり、ひとつのエラーで止まらないでできるだけ多くのエラーを見つけられるようにしたりするために工夫しますが、Toilでは単純に済ませます。
...
ASTはAbstract Syntax Tree（抽象構文木）の略語で、テキストによるソースコードを、コンピュータで扱いやすい「木」の形に変換したものです。
...
`eval()`ではこの「木」をあちこち歩き回りながら評価していくので、このような処理系をツリーウォーク式インタプリタ（Tree Walk Interpreter）と呼びます。ちょっと長いので本書ではTWIと表記します。

これから、いろいろなASTに対する処理を付け加えて評価器を育てていきます。
...
オレオレルールですがToilのソースでは
1行で開いた分のカッコは1行で閉じる（後に `do` ... `end` のような構文を導入したときはそれを含めて）
...
Pythonの`if`では`else`がなかったり`elif`で続いたりすることもありますが`eval()`ではこの形のみ扱います。
...
です。
条件式、帰結式、代替式と呼ぶとなんだか難しそうに聞こえると思いますので本書ではCond式、Then式、Else式と呼んでいきます。
...
[^javascript-call]: 主にJavaScriptを思い浮かべながら書いています。https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Functions/Default_parameters あたり。JavaScriptの関数呼び出しはこれだけじゃなくていろんなおもしろ（または変態）機能が満載です。おもしろいんですけどお仕事で使うときはあまりうれしくないらしいです。
...
ステップ1の時点で、`params`は`["a", "b"]`、`body_expr`は`("add", ["a", "b"])`、`args_val`は`[2, 3]`になっています。
ローカル変数用のスコープを作ったら、次にステップ2で引数の処理を行います。
...
繰り返しの1周目ではまず配列の先頭の要素`("a", 2)`を`params, arg`に割り当てるので`param`が`"a"`、`arg`が`2`になります。
その状態で`for`の本体（`new_env.define(param, arg)`）を実行するので、名前が`"a"`で値が`2`の変数ができます。
2周目では`params`が`"b"`、`arg`が`3`になって同様に名前が`"b"`で値が`3`の変数ができます。

[^idiomatic-zip]: よく使われるイディオムなんですが弱点がありまして、配列の長さが違っていると、短いほうが終わったところで`for`も終わってしまいます。ということはつまり、Toilでは仮引数と実引数の数が違っていると、長いほうは余ってしまうのです。その後どうなるかは神のみぞ知るです。まじめな言語を作るときはマネしないでちゃんと考慮しましょう。前節の注にも関連しますが、チェックしてエラーにするだけが対応方法ではありません。
...
なぜでしょうか？
このとき環境はこうなっていて、`f()`を呼び出したスコープで`"a"`が見つかるので3を返します。
...
このコードを実行すると3が返ってきてしまってバグの元、というのが前節のラストでした。
...
前節の`make_counter`関数も、Pythonではこう書く必要があります。残念[^one-more-shame]。
...
[^one-more-shame]: このコードには`lambda`が複数行にわたって書けなくて`def`でわざわざ名前を付けて関数を作る必要がある、というもうひとつのPythonの残念な点も現れています。`nonlocal`は文なのでそもそも`lambda`に書けないし・・・
...
以下のように、スコープに入れないで`"a"`を変更してしまうとこれは3が返ってしまいます。
作ったときの環境を覚えているとはいえ、その環境の中の2が上書きされてしまっているので。
...
PythonやToilではそれもないのでユーザが自分でスコープを切ってやることになります。
Pythonは単独でスコープを作る機能もないので関数を分けたり他の手段を考えたりする必要があります。

`for`や`if`で自動的にスコープを作るようにするのは簡単で、`new_env = Environment(env)`してやればいいのですが
`env`の段数が増えてデバッグ時に見づらいというのもあってPythonに寄った仕様にしています。

### whileを評価する

はい、繰り返し（while）を作ってませんでしたね！
静的スコープを大モノ扱いしたのでwhileが小モノみたいな扱いになってしまってますが
順次・選択に加えて繰り返しがあればすべてのアルゴリズムが書けるということになっているので
やはり大事なものです[^recursive-school]。

[^recursive-school]: ifと関数があれば再帰で繰り返しが書けるから繰り返しなんかいらないよ、という宗派もあります。

whileは制御構造なので`seq`や`if`と同じように書いていきます。
...
`i`が4より小さい間`i`を`print`します。
`1`、`2`、`3`はprintの実行結果で、`4`はwhile文の返した値、つまり`("assign", ["i", ("add", ["i", 1])])`を最後に実行したときの値です。
...
whileの本体が一度も実行されなかったときは`None`を返します。
whileの本体は`("div", [1, 0])`で、もし実行されてしまったら`ZeroDivisionError`になります。
...
これで、第2章で実装予定の機能をすべて実装しました！
第2章の最後に、この評価器でどのようなプログラムが書けるようになったか
...
まずはnの階乗の計算をwhileを使った形で書いてみます。
nの階乗とは、1からnまでの数をかけ算したものです。
この例ではnから1までかけ算する、という形に書いていますが。
0の階乗は1です。とりあえずそういうものだと思ってください。

比較用のPythonのコードを併記します。
...
次は同じnの階乗を再帰を使って書いてみます。
計算結果は同じですが、考え方の方向が大きく違います。再帰では以下のように考えます。

* nが0のとき、nの階乗は1
* nが0より大きいとき、nの階乗は(n - 1)の階乗にnをかけたもの

どういうことかな、と思ったらnを3とかにして実際に計算してみてください。

3の階乗 = (3 - 1)の階乗 × 3 = 2の階乗 × 3
2の階乗 × 3 = (2 - 1)の階乗 × 2 × 3 = 1の階乗 × 2 × 3
1の階乗 × 2 × 3 = (1 - 1)の階乗 × 2 × 3 = 0の階乗 × 2 × 3
0の階乗 × 2 × 3 = 1 × 2 × 3

なので「1からnまでの数をかけ算したもの」と同じ結果になっていますね。

Pythonのほうではlambdaとif式を使って書いてみました。
こちらのほうが関数を作っておいて変数に割り当てる、というToilの考え方に近い書き方になります。
...
Toilにはまだ「列」を簡単に扱う方法がないので、「直前の二つの数」だけを覚えておく書き方でn番目のフィボナッチ数を計算します。
...
* nが0のとき、n番目のフィボナッチ数は0
* nが1のとき、n番目のフィボナッチ数は1
* nが2以上の時、n番目のフィボナッチ数は(n - 1)番目のフィボナッチ数と(n - 2)番目のフィボナッチ数を足したもの
...
以下の箇所では、if-elif-else風なことを書いています。
...
elifはなくても書き方でなんとかなる、とも言えます。

次は最大公約数（Greatest Common Divisor: GCD）を求める互除法というアルゴリズムです。
互除法については説明しませんが、Pythonのコードと見比べてToilでも同じように書けていることを見てください。
...
Pythonの例で`return a`と書いているように、最終的な`a`の値が最大公約数を表しています。
Toilの例では`while`本体の最後の式`("assign", ["a", "tmp"])`が`a`の値を返しますので
...
* nが0のときnは偶数
* nが0より大きいとき、n - 1が奇数ならnは偶数
* nが0より大きいとき、n - 1が偶数ならnは奇数
...
「なんでもできる」は言い過ぎでしょ？数とboolとNoneしかなくて、データ構造も作れないし、という人もいるかもしれません[^lambda-calculus]。

[^lambda-calculus]: 実はboolや数がなくても関数だけでそれら（に相当するもの）が（がんばれば）作れます。「ラムダ計算」で調べてみてください。

いえいえ、クロージャがあるとデータ構造が作れてしまうのです。
その証拠に二分探索木（Binary search tree）を作ります。
ここはToilがすでにデータ構造を作る能力を持っているということを示すのが目的なので、二分探索木については説明しません。
...
Pythonでは`n1`というオブジェクトを作って`n1.val`で`n1`の値を取り出しています。
Toilでは`n1`という関数を作って`("n1", [1])`という呼び出しで`n1`の値を取り出します。
`("node", ...)`と呼び出すたびに別のクロージャが作成され、
それぞれの環境にローカル変数`val`、`left`、`right`が作られるので、
たくさんのノードを別々に作ることができます。

あとは二分探索木のアルゴリズムを記述していくだけです。
アルゴリズムの解説やPythonのコードは省略します。
...
二分探索木の中に値が含まれるかどうかを返す関数を作り、0から9までの値について調べます。
...
一度、評価器のコード全体を眺めなおして、実行例を除けばたった100行程度のPythonプログラムで、いろいろなアルゴリズムが書けていることに驚いてください。
これだけでも大きな学びになっているはず。
...
これは、LISPと言われるプログラミング言語（のファミリー）で使われる書き方[^just-a-sample]で、S式（S-expression）と言います。

英語っぽいPythonやその他の言語と比較すれば少々人間にはやさしくないところもありますが、コンピュータには処理しやすい形となっています。
人間に見やすいExcelよりも、CSVのほうが集計はしやすいというのと似ていますね。
おかげでLISPではプログラム自体を操作するプログラムを書くことが容易で、言語のユーザが言語に機能を追加することができるようになっています。
本書では第4部で体験します。

著者は言語処理系の学習をLISP系の書籍で始めたので、考え方がLISP派寄りです。
評価器の実装はLISPの学習のまとめとしてよくとりあげられる題材で、第2章で作った評価器もこれに近いものです。

[^just-a-sample]: 書き方だけで、このままではLISPの処理系で実行することはできません。ちょっと直す程度で動きますが。

「配列」を「リスト」と呼ぶのに抵抗を感じているのはここに根っこがありまして、LISPの`(define factorial_rec (func (n) ...`という式はいわゆる「線形リスト」（次の要素をポインタで指すことによって要素の並びを表現するデータ構造）で表されていて、普通は単に「リスト」と呼んでおり、ソースコードが「リスト」であることがLISPの重要な特徴である、からです。

といってもPythonの世界の外では、データ構造で「リスト」といったらを線形リスト指すのが一般的だと思います。きっと。Pythonに本当の「リスト」があればToilのASTもそれで書いたのになあ・・・
...
これまでに作った `scan`（字句解析）、`parse`（構文解析）、`eval`（評価）の3つの処理をつないで、テキストのソースコードを実行するインタプリタにします。
...
文字列のほうをToilStringみたいなクラスで表すという手もあるけど
...
GitHubのソースコードで丸ごと上書きするか、なんだかそれはヤダという人（わかる）は正規表現の置換である程度は対応できます
AIにやってもらう手もありますが完璧にはやってもらえませんでした
VS Codeでは
...
引数を評価しないだけのfmacro()だけ作る
後で事前展開型のマクロを実装するが、TWIにはfmacroも残して共存させる。ICIではfmacroは実装せず、TWI専用の機能とする。
式はIdentやtupleを使って組み立てる

この段階ではmacroという名前で作っておき、事前展開型のマクロを作ったときにevaluatorでfmacroという名前にすれば
それまでmacroと書いてあったところが自動的に事前展開型のマクロで処理されるようになるか？
実装中、一時的に動かなくなりそう。最初からfmacroという名前にしておき、
...
fmacro同様、quoteをfquoteなど別名にすればTWI専用の機能として残しておけるか？
