...
# 100行のPythonから育てて学ぶプログラミング言語の作り方
...
#### 本物のインタプリタ 0203_real_interpreter
...
Result:
5
……はい。5を入れたら5が返ってきました。それだけです。 a のような文字を入れると怒られます。 「せっかくREPLまで作ったのに、数字のオウム返ししかできないの！？」と思うかもしれません。でも、いいんです。タプルを手書きしていた時代を思い出してください。これは間違いなく、文字列のソースコードを読み取って実行する「本物のプログラミング言語」になった証です。

最高のテスト環境は整いました。次節からは、この空っぽの脳みそに「足し算」や「変数」といった知識をどんどん教え込んでいきますよ！
...
もちろん、前節で作成した REPL（./toil.py --repl）を使って実行することも可能です。紙面の都合上、今後は Python コードでの実行例を中心に記載しますが、皆さんはぜひ手元の REPL を立ち上げて、いろいろな入力を試しながら読み進めてください！


現時点ではprintもなにもないので実行しても何も起きないように見えますが
...
始めと終わりがはっきりしてるという意味で大きなカッコみたいなものなので_primary()で読みます
構文解析しやすい構文にしています
