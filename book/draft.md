...
* 第1部 最小のインタプリタ
  ...
  * 第4章：最小の中間コードインタプリタ [^bytecode-interpreter]  (500行)
    第3章の字句解析・構文解析器とASTを中間コードに翻訳するコンパイラ、その中間コードを実行するVMの組み合わせによるインタプリタを作ります。
...
実行例を除けばたった100行程度のPythonプログラムですが、いろいろなプログラムが実行できるようになりました。
これだけでも大きな学びになっているはず。
...
[^just-a-sample]: 書き方だけで、このままではLISPの処理系で実行することはできません。ちょっと直す程度ですが。
