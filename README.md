...
* ただし、状態を持たない関数群であっても、当初からクラスとして実装する。
  * 関心の分離を明確に表す。
  * 名前空間をクリーンに保つ。
  * 一貫性を持つ。
...
* [x] 最小のツリーウォーク式評価器を実装する。
  * [x] 値を実装する。扱う値はNone、bool、intのみ。
  * [x] 複数の式を順番に評価するseq式を実装する。
  * [x] if式を実装する。
  * [x] 変数（定義・参照）を評価する。まずはスコープなしで実装する。
  * [x] スコープを実装する。
    * 当初は動的スコープで実装し、後に静的スコープ（クロージャ）へ移行する。
  * [x] 組み込み関数（add・sub・mul・equal・print）、ユーザ定義関数、クロージャ
  * [x] 再帰でフィボナッチ数が求められる。
