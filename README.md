...
* **UFCS (Unified Function Call Syntax)**:
  * `obj.func(arg)` のようなドット記法の呼び出しにおいて、`obj` 自身が `func` というキーを持たない場合、自動的に第一引数として展開され `func(obj, arg)` としてグローバル関数が呼び出される。
  * `func`の第一引数が`self`の場合、`self`には`obj`が代入されるので、メソッド呼び出しのようにも書ける。
  * これにより、オブジェクト指向的なメソッド呼び出しや、直感的なデータ変換パイプラインを簡潔に記述できます。
* **標準ライブラリ**:
