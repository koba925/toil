...
* 辞書・オブジェクト指向（風記法）・モジュールを導入する。
  ...
  * defmoduleを実装する。
    * 言語機能には組み込まないかもしれない（セルフインタプリタに必要としないため）。
    * 例としてはおもしろいのでやりたいのはやりたい
* パターンマッチを導入する。
  * 分割代入を実装する。
    * rest引数`[a, *rest]`を書けるようにする。rest引数は末尾のみ許可する。
    * _（ワイルドカード）も疑似的にサポートする。
    * forでも動作することを確認する。
  * 引数パターンマッチを実装する。
  * パターンマッチ構文を実装する。
    * リテラルパターンを追加する。
  ...
  * 例外処理を実装する。
* セルフインタプリタを実装する。
  * マクロは実装しないでパターンマッチまでをサポートしたサブセットとする。
* マクロ実装する パート1：ファーストクラスマクロとカスタム文法
  * マクロ実装の準備を行う。
    * Ident()、tuple()で識別子・式（タプル）を直接生成できるようにする。
      * パターンマッチにも追加する。
  ...
  * カスタム文法を実装する。
    ...
    * defmoduleを実装する。サンプルのみで言語機能には組み込まない。
* マクロを実装する パート2：事前展開型マクロ
    * parseとevaluateの間にexpandフェーズを入れてマクロを事前に展開するようにする。
    * カスタム文法はパート1のコードからコピーする。
      * それともパート2のマクロができてからの方がよいか？カスタム文法がないとマクロがいまひとつありがたみに欠けそうだが。
...
* ユーザ自身の学習のため、言語処理系のコードはユーザが書きます。提案されない限り先回りしてコードを出力することを固く禁じます。
* 謝罪、賞賛、挨拶などの不要な会話を省く。
